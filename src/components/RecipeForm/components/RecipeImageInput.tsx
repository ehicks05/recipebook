...
const MyUppy = ({ recipeId, token }: MyUppyProps) => {
  const {refetch: refetchRecipe} = api.example.findRecipe.useQuery({ id: recipeId });
  ...
  return (
    <div className="flex flex-col gap-1">
      <label>
        ...
        {recipe?.imageSrc ? (
          <Image src={recipe.imageSrc} alt="recipe" onClick={() => setIsOpen(true)} className="rounded-lg cursor-pointer hover:animate-pulse" width={300} height={300} />
          ) : (
